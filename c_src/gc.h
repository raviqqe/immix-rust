...
inline void process_edge(Address addr, AddressVector *work_queue,
                         uint8_t mark_state) __attribute__((always_inline));
inline void process_edge(Address addr, AddressVector *work_queue,
                         uint8_t mark_state) {
...
void trace_object_no_inline(ObjectReference obj, AddressVector *work_queue,
                            uint8_t mark_state, LineMark *line_mark_table,
                            int line_mark_table_len, Address space_start,
                            Address space_end);

inline void trace_object(ObjectReference obj, AddressVector *work_queue,
                         uint8_t mark_state, LineMark *line_mark_table,
                         int line_mark_table_len, Address space_start,
                         Address space_end) __attribute__((always_inline));
inline void trace_object(ObjectReference obj, AddressVector *work_queue,
                         uint8_t mark_state, LineMark *line_mark_table,
                         int line_mark_table_len, Address space_start,
                         Address space_end) {
  ...
  while (true) {
    ...
    case 0b00001111:
      ...
      process_edge(base + 24, work_queue, mark_state);
      break;
    default:
      printf("unexpected ref_bits patterns");
      exit(1);
    }
    ...
    if (short_encode) {
      ...
      printf("should not use long encode");
      exit(1);
