...
ImmixMutatorLocal *ImmixMutatorLocal_new(ImmixSpace *space) {
  ImmixMutatorLocal *ret =
      (ImmixMutatorLocal *)malloc(sizeof(ImmixMutatorLocal));
...
Address ImmixMutatorLocal_alloc_from_global(ImmixMutatorLocal *mutator,
                                            int size, int align) {
...
Address ImmixMutatorLocal_try_alloc_from_local(ImmixMutatorLocal *mutator,
                                               int size, int align) {
  if (mutator->line < IMMIX_LINES_IN_BLOCK) {
    int next_available_line =
        ImmixBlock_get_next_avaiable_line(mutator->block, mutator->line);

    if (next_available_line != -1) {
      int end_line = ImmixBlock_get_next_unavailable_line(mutator->block,
                                                          next_available_line);

      mutator->cursor = mutator->block->start +
                        (next_available_line << IMMIX_LOG_BYTES_IN_LINE);
      mutator->limit =
          mutator->block->start + (end_line << IMMIX_LOG_BYTES_IN_LINE);
...
void init_object_no_inline(ImmixMutatorLocal *mutator, Address addr,
                           uint8_t encode) {
  mutator->alloc_map[(addr - mutator->space_start) >> LOG_POINTER_SIZE] =
      encode;
}
