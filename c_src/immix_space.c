...
void init_blocks(ImmixSpace *space) {
  ...
  for (; cur < space->end; cur += IMMIX_BYTES_IN_BLOCK,
                           curLineMark = &(curLineMark[IMMIX_LINES_IN_BLOCK]),
                           id += 1) {
...
ImmixSpace *ImmixSpace_new(int space_size) {
  // mmap
  void *mmap_ret =
      mmap(0, space_size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANON, -1, 0);
  ...
  ret->line_mark_table =
      (LineMark *)malloc(sizeof(LineMark) * ret->line_mark_table_len);
