...
void alloc_loop(ImmixMutatorLocal *mutator) {
  ...
  for (; i < ALLOCATION_TIMES; i++) {
    ...
    Address res =
        ImmixMutatorLocal_alloc(mutator, ACTUAL_OBJECT_SIZE, OBJECT_ALIGN);
  ...
  double duration = (double)(t_end.tv_usec - t_start.tv_usec) / 1000 +
...
void mark_loop(Address *objs, ImmixSpace *immix_space) {
  ...
  for (; i < MARK_TIMES; i++) {
    ...
    if (obj >= space_start && obj < space_end) {
      mark_line_live(line_mark_table, line_mark_table_len, space_start,
                     objs[i]);
    }
  }
