...
// extern Type* types;
//
// Header Header_new(uint8_t gcbits, uint8_t ref_bits, uint16_t size, uint32_t
// ty_id); void Header_print(Header* header);
//
...
// __attribute__((always_inline)); inline void set_gcbits_as(ObjectReference obj,
...
// __attribute__((always_inline)); inline bool test_gcbits(ObjectReference obj,
// int index, uint8_t value) {
...
inline bool is_traced(ObjectReference obj, uint8_t mark_state) {
  return immix_space
             ->trace_map[(obj - immix_space->start) >> LOG_POINTER_SIZE] ==
         mark_state;
}

inline void mark_as_traced(uint8_t *trace_map, ObjectReference obj,
                           Address space_start, uint8_t mark_state)
    __attribute__((always_inline));
inline void mark_as_traced(uint8_t *trace_map, ObjectReference obj,
                           Address space_start, uint8_t mark_state) {
