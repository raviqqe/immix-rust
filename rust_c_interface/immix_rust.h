...
// the layout of this struct needs to be the same as
// src/heap/immix/immix_mutator.rs
struct Mutator {
  uint64_t id;
  void *alloc_map;
...
extern void gc_init(uint64_t immix_size, uint64_t lo_size,
                    uint64_t n_gcthreads);
...
extern uint64_t alloc_slow(struct Mutator **mutator, uint64_t size,
                           uint64_t align);
